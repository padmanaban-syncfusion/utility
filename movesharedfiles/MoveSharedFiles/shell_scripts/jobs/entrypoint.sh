...
if [ -d "$clientLibraryPath" ]; then
    bash $clientLibraryPath/install-optional.libs.sh install-optional-libs jobs
    echo "Clientlibrary installed from boldreports folder"
else
    bash $commonPath/reporting/install-optional.libs.sh install-optional-libs jobs
    echo  "Clientlibrary installed from reporting folder"
fi

dotnet Syncfusion.Server.Jobs.dll